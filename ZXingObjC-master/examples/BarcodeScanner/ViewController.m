...
@interface ViewController ()<ZXCaptureDelegate,UIGestureRecognizerDelegate>
...
- (void)viewDidLoad {
    ...
    //
    //   SNCoreScaner *coreScaner = [[SNCoreScaner alloc]init];
    //    [coreScaner  startReader:YES];
    //
...
- (void)zoomChangePinchGestureRecognizerClick:(UIPinchGestureRecognizer *)pinchGestureRecognizer{
    ...
    if (pinchGestureRecognizer.state == UIGestureRecognizerStateBegan ||
        ...
        if (pinchGestureRecognizer.scale>1) {
            self.currentZoomFactor += pinchGestureRecognizer.scale;
        }else{
            self.currentZoomFactor -= pinchGestureRecognizer.scale;
        }
        
        NSLog(@"pinchGestureRecognizer.scale>> %lf",pinchGestureRecognizer.scale);
        
        //        self.currentZoomFactor = currentZoomFactor;
        
        NSLog(@"currentZoomFactor>> %lf",self.currentZoomFactor);
        
        if (self.currentZoomFactor < self.maxZoomFactor &&
            self.currentZoomFactor > self.minZoomFactor){
            
            
            self.capture.transform = CGAffineTransformMakeScale(self.currentZoomFactor , self.currentZoomFactor);
